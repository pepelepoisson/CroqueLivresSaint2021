...
void displayDashboard(GxEPD2_GFX& display, int code_run_counter, int current_citation, int number_of_citations, float batteryVoltage){
  //display.fillScreen(GxEPD_WHITE);
  ...
  do
  {
    ...
    display.setFont(&FreeMonoBold9pt7b);
    display.setCursor(5, 10);
    display.print("Ex}cutions: ");
    display.setFont(&FreeMonoBold24pt7b);
    display.setCursor(5, 80);
    display.println(String(code_run_counter));
    //display.setCursor(5, 30);
    //display.print("Citation actuelle: ");display.println(String(current_citation));
    display.setFont(&FreeMonoBold9pt7b);
    display.setCursor(148, 10);
    display.print("Batterie (V): ");
    display.setFont(&FreeMonoBold24pt7b);
    display.setCursor(148, 80);
...
void setup()
{
  ...
  // If door is opened on startup then battery voltage can be measured. Do so and calculate battery voltage by averaging 25 readings.
  if (analogRead(analogInPin)>100){
    for(int i=0; i<25; i++) {sensorValue = sensorValue+analogRead(analogInPin);delay(10);}
    batteryVoltage=sensorValue/(batteryVoltageCalibration*25);
...
void loop()
{
  ...
  if (sensorValue>100){doorOpen=true;}
  else {doorOpen=false;}

  if ((millis()-start_time>LED_ANIMATIONS_DURATION)||!doorOpen){
    LED("off");
    if (doorOpen==true){Serial.print(" battery voltage: "); Serial.print(batteryVoltage);Serial.println("V");}
    delay(50);
    display1.init(115200); // enable diagnostic output on Serial
    drawBitmaps(display1);
    delay(2000);

    displayDashboard(display1,updated_code_run_counter, updated_current_citation,number_of_citations, batteryVoltage);
    delay(2000);
    ...
    //print_string(display1,citation.replace("é","{"));
    //print_string(display1,"abcde { | } ~ abc abcde { | } ~ abc abcde { | } ~ abc v abcde { | } ~ abc abcde { | } ~ abc abcde { | } ~ abc abcde { | } ~ abc");
    delay(500);
    display1.powerOff();
